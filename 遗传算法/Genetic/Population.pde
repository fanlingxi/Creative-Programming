...
class Population {
  ...
  void selection( ArrayList<Rocket> matingPool, int maxcount, float maxFitness, ArrayList<PVector[]> maxTrail ) {
    // Clear the ArrayList
    for( ;matingPool.size()>maxcount; ){
      matingPool.remove( 0 );
    }
    //matingPool.clear();
    ...
    for (int i = 0; i < population.length; i++) {
      ...
      if(population[i].getFitness() >= maxFitness){
          maxTrail.add( new PVector[population[i].trail.size()] );
          for( int f=0; f<population[i].trail.size(); f++ )  maxTrail.get(maxTrail.size()-1)[f] = population[i].trail.get(f).copy();
      }
  ...
  void reproduction( ArrayList<Rocket> matingPool ) {
